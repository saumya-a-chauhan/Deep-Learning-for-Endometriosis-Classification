...
#
# 2. Place this script in a directory, e.g., "MyProject"
#
...
#    python train.py
#
# 5. All results will be saved to "model_results.txt"
...
def main():
    # --- Configuration ---
    DATA_DIR = Path("dataset_final") # <-- YOUR RELATIVE PATH
    OUTPUT_FILE = "model_results.txt"
    TRAIN_PROPORTIONS = [0.1, 0.2, 0.5]
    ...
    # --- Load Full Datasets ---
    # We need two dataset objects: one for training (with augmentations)
    # and one for validation/testing (without augmentations).
    try:
    ...
    if len(class_names) != 2:
        print(f"Warning: Expected 2 classes, but found {len(class_names)}: {class_names}")
    ...
    with open(OUTPUT_FILE, 'w') as f:
        ...
        with redirect_stdout(f):
            print(f"Starting experiments. Device: {DEVICE}")
            print(f"Data source: {DATA_DIR.resolve()}")
            print(f"Found {len(targets)} total images. Classes: {class_names}\n")
            ...
            for prop in TRAIN_PROPORTIONS:
                ...
                # This splits the *entire* dataset into a train/val pool (prop) and a test pool (1-prop)
                train_val_idx, test_idx = train_test_split(
                    indices, 
                    train_size=prop,  # Use 'train_size' to match 10%, 20%, 50%
                    stratify=targets, 
                    random_state=SEED, 
                    shuffle=True
                )
                ...
                # This splits the 'train_val_pool' into its own 80% train / 20% val sets
                ...
                if len(set(train_val_targets)) < 2:
                     ...
                     # Fallback to non-stratified split if only one class (can happen with tiny splits)
                     train_idx, val_idx = train_test_split(
                        train_val_idx,
                        test_size=0.20, # 20% of the train_val pool
                        random_state=SEED,
                        shuffle=True
                     )
                else:
                ...
                train_ds = Subset(dataset_full_train, train_idx) # Uses train transforms
                val_ds = Subset(dataset_full_eval, val_idx)       # Uses eval transforms
                test_ds = Subset(dataset_full_eval, test_idx)      # Uses eval transforms
